...
// ---- storage keys for this card's view prefs ----
var BM_KEYS = {
...
// stable pastel accent from a tag name
...
function BookmarksHero() {
  ...
  var refresh = useCallback(async function () {
    ...
    (function walk(nodes) {
      for (var i = 0; i < nodes.length; i++) {
        var node = nodes[i];
        if (node.children) { if (node.id !== "0") { f.push(node); fc++; } walk(node.children); }
  ...
  var toggleExpand = function (e, id) {
    e.stopPropagation();
    var next = expandedFolders.indexOf(id) >= 0 ? expandedFolders.filter(function (x) { return x !== id; }) : expandedFolders.concat([id]);
    setExpandedFolders(next);
    window.setStorage(window.STORAGE_KEYS.treeExpanded, next);
  };
  var toggleCollapse = function (tag) {
    var next = collapsed.indexOf(tag) >= 0 ? collapsed.filter(function (t) { return t !== tag; }) : collapsed.concat([tag]);
    setCollapsed(next);
  ...
  var matches = function (b) {
    ...
    return (b.title || "").toLowerCase().indexOf(q) >= 0 ||
           ...
           normalizeTags(meta[b.id]).join(" ").toLowerCase().indexOf(q) >= 0;
  };

  // ----- tier 1: pinned -----
  var pinned = bookmarks.filter(function (b) { return meta[b.id] && meta[b.id].pinned; });
  ...
  var folderDescendantIds = useMemo(function () {
    ...
    var result = {};
    folders.forEach(function (f) {
      var ids = new Set([f.id]);
      var stack = (childMap[f.id] || []).slice();
      while (stack.length) {
        var x = stack.pop();
        if (ids.has(x)) continue;
        ids.add(x);
        (childMap[x] || []).forEach(function (c) { stack.push(c); });
      }
      result[f.id] = ids;
    });
    return result;
  ...
  var treeFiltered = useMemo(function () {
    ...
    if (activeFolderId !== "all") {
      var allowed = folderDescendantIds[activeFolderId] || new Set([activeFolderId]);
      items = items.filter(function (x) { return allowed.has(x.parentId); });
    }
  ...
  // ----- tier 2: grouped by tag -----
  var groups = useMemo(function () {
    var map = {};        // tag -> bookmarks[]
    var order = [];      // tag insertion order
    var untagged = [];
    bookmarks.forEach(function (b) {
      if (!matches(b)) return;
      var tags = normalizeTags(meta[b.id]);
      if (tags.length === 0) { untagged.push(b); return; }
      tags.forEach(function (t) {
        if (!map[t]) { map[t] = []; order.push(t); }
        map[t].push(b);
      });
    });
    order.sort(function (a, b) { return map[b].length - map[a].length || a.localeCompare(b); });
    var out = order.map(function (t) { return { tag: t, items: map[t] }; });
  ...
  return h("div", { className: "card hero-card bm-hero", "data-shape": shape },
    ...
    h("div", { className: "card-head bm-head" },
      h("div", { className: "bm-title-wrap" },
        ...
        h("span", { className: "bm-sub" }, bookmarks.length + " saved \u00B7 " + folderCount + " folders")
      ),
      h("div", { className: "bm-head-actions" },
        h("div", { className: "bm-search" },
          h(Icon.search, { size: 15, style: { color: "var(--text-mute)", flexShrink: 0 } }),
          h("input", {
            type: "text", placeholder: "Search bookmarks\u2026", value: query,
            onChange: function (e) { setQuery(e.target.value); },
          })
        ),
        h("div", { className: "bm-viewtoggle" },
          h("button", {
            className: "bm-vt-btn" + (view === "tree" ? " active" : ""), title: "Folder tree",
            onClick: function () { setViewP("tree"); },
          }, h(Icon.folder, { size: 16 })),
          h("button", {
            className: "bm-vt-btn" + (view === "card" ? " active" : ""), title: "Card view",
    ...
    pinned.length > 0 && h("div", { className: "bm-quick" },
      h("div", { className: "bm-quick-grid" },
        pinnedShown.map(function (b) {
          return h("div", { key: b.id, className: "qtile-wrap" },
            h("a", { href: b.url, className: "qtile", title: b.title + " \u2014 " + hostOf(b.url) },
              h("span", { className: "qtile-logo", style: { background: (meta[b.id] && meta[b.id].color) || "var(--surface-2)" } },
                h(window.BookmarkIcon, { url: b.url, title: b.title, meta: meta[b.id] })
              ),
              showLabels && h("span", { className: "qtile-name" }, b.title)
            ),
            h("button", {
              className: "qtile-unpin", title: "Unpin",
              onClick: function (e) { e.preventDefault(); e.stopPropagation(); togglePin(b.id); },
            }, h(Icon.close, { size: 11 }))
          );
        })
      ),
      pinned.length > pinnedShown.length && !q && h("div", { className: "bm-quick-more" },
        "+" + (pinned.length - pinnedShown.length) + " more pinned \u00B7 the quick strip shows your top " + Math.min(cap, 10) + " \u2014 unpin to swap")
    ),

    // ----- folder tree view -----
    view === "tree" && h("div", { className: "bm-tree-layout" },
      h("div", { className: "bm-tree" },
        h("div", {
          className: "tree-node tree-node-all" + (activeFolderId === "all" ? " selected" : ""),
          onClick: function () { setFolderP("all"); },
        },
        ...
        folders.filter(function (f) { return f.parentId === "0"; }).map(function (f) {
          return h(TreeNode, {
            key: f.id, node: f, depth: 0, activeId: activeFolderId,
            onSelect: setFolderP, expandedIds: expandedFolders, onToggle: toggleExpand,
          });
        })
      ),
      h("div", { className: "bm-list-pane" },
        treeFiltered.map(function (b) {
          return h(BookmarkRow, {
            key: b.id, bookmark: b, meta: meta[b.id],
            pinned: !!(meta[b.id] && meta[b.id].pinned),
            onTogglePin: function () { togglePin(b.id); },
          });
        }),
        treeFiltered.length === 0 && h("div", { className: "bm-empty" },
          q ? "No bookmarks match \u201C" + query + "\u201D" : "This folder is empty")
      )
    ),

    // ----- tier 2: tag groups (grid / card) -----
    view !== "tree" && h("div", { className: "bm-groups" },
      groups.map(function (g) {
        var isCol = collapsed.indexOf(g.tag) >= 0;
        var hue = g.untagged ? null : tagHue(g.tag);
        return h("section", { key: g.tag, className: "bm-group" },
          h("button", {
            className: "bm-group-head" + (isCol ? " collapsed" : ""),
            onClick: function () { toggleCollapse(g.tag); },
          },
            h("span", { className: "bm-group-chev" }, h(ChevronGlyph, null)),
            g.untagged
              ? h("span", { className: "bm-group-dot untagged" })
              : h("span", { className: "bm-group-dot", style: { background: "hsl(" + hue + " 65% 55%)" } }),
            h("span", { className: "bm-group-name" }, g.tag),
            h("span", { className: "bm-group-count" }, g.items.length)
          ),
          !isCol && h("div", { className: "bm-group-grid" + (view === "grid" ? " grid-mode" : "") },
            g.items.map(function (b) {
              return h(BookmarkCard, {
                key: g.tag + ":" + b.id, bookmark: b, meta: meta[b.id], view: view,
                pinned: !!(meta[b.id] && meta[b.id].pinned),
                onTogglePin: function () { togglePin(b.id); },
              });
            })
      ...
      totalShown === 0 && h("div", { className: "bm-empty" },
...
function ChevronGlyph() {
  return h("svg", { width: "10", height: "10", viewBox: "0 0 10 10", fill: "none", stroke: "currentColor", strokeWidth: "1.8", strokeLinecap: "round", strokeLinejoin: "round" },
    h("polyline", { points: "3.5 2 6.5 5 3.5 8" }));
}

function BookmarkCard(props) {
  var b = props.bookmark, meta = props.meta, pinned = props.pinned;
  var host = useMemo(function () { return hostOf(b.url); }, [b.url]);
  var pinBtn = h("button", {
    className: "bcard-pin" + (pinned ? " active" : ""),
    title: pinned ? "Unpin from quick access" : "Pin to quick access",
    onClick: function (e) { e.preventDefault(); e.stopPropagation(); props.onTogglePin && props.onTogglePin(); },
  }, h(Icon.pin, { size: 13 }));

  if (props.view === "grid") {
    return h("div", { className: "bcard-wrap grid" },
      h("a", { href: b.url, className: "bcard bcard-grid", title: b.title + " \u2014 " + host },
        h("span", { className: "bcard-logo", style: { background: (meta && meta.color) || "var(--surface)" } },
          h(window.BookmarkIcon, { url: b.url, title: b.title, meta: meta })
        ),
  ...
  return h("div", { className: "bcard-wrap" },
    h("a", { href: b.url, className: "bcard", title: b.title },
      h("span", { className: "bcard-logo", style: { background: (meta && meta.color) || "var(--surface)" } },
        h(window.BookmarkIcon, { url: b.url, title: b.title, meta: meta })
      ),
      h("span", { className: "bcard-text" },
...
function TreeNode(props) {
  ...
  var subFolders = (node.children || []).filter(function (c) { return c.children; });
  var directCount = (node.children || []).filter(function (c) { return c.url; }).length;
  var hasChildren = subFolders.length > 0;
  return h("div", { className: "tree-group" },
    h("div", {
      ...
      h("span", {
        ...
        onClick: function (e) { e.stopPropagation(); if (hasChildren) props.onToggle(e, node.id); },
      }, h(ChevronGlyph, null)),
    ...
    isExpanded && hasChildren && h("div", { className: "tree-children" },
      subFolders.map(function (c) {
        return h(TreeNode, {
          key: c.id, node: c, depth: depth + 1, activeId: activeId,
          onSelect: props.onSelect, expandedIds: props.expandedIds, onToggle: props.onToggle,
        });
      })
    )
  );
...
function BookmarkRow(props) {
  ...
  return h("div", { className: "bm-row-wrap" },
    h("a", { href: b.url, className: "bm-row", title: b.title },
      h("span", { className: "bm-row-logo", style: { background: (meta && meta.color) || "var(--surface-2)" } },
        h(window.BookmarkIcon, { url: b.url, title: b.title, meta: meta })
      ),
      h("span", { className: "bm-row-main" },
        h("span", { className: "bm-row-name" },
          b.title,
          tags.map(function (t) { return h("span", { key: t, className: "bm-row-tag" }, t); })
        ),
